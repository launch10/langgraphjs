...
describe.each(storageTypes)("Auth tests (%s storage)", (storageType) => {
  const port = storageType === "postgres" ? 2026 : 2027;
  ...
  it.skipIf(process.version.startsWith("v18."))(
    ...
    async () => {
      ...
      await expect(
        user.assistants.create({ graphId: "agent" })
  ...
  it.skipIf(process.version.startsWith("v18."))(
    ...
    async () => {
      ...
      const user1 = await createJwtClient("johndoe", [
        "me",
        "assistants:write",
      ]);
  ...
  it.skipIf(process.version.startsWith("v18."))("store auth", async () => {
    ...
    await expect(
      userA.store.listNamespaces({ prefix: ["ALL"] })
    ).rejects.toThrow("HTTP 403");
    ...
    expect(
      await userB.store.listNamespaces({ prefix: ["alice"] })
    ).toMatchObject({ namespaces: [["alice"]] });
  });
  ...
  it.skipIf(process.version.startsWith("v18."))(
    ...
    async () => {
      ...
      await expect(
        otherUser.threads.getState(thread.thread_id)
      ).rejects.toThrow("HTTP 404");
      await expect(
  ...
  it.skipIf(process.version.startsWith("v18."))(
    ...
    async () => {
      const owner = await createJwtClient("johndoe", [
        "me",
        "assistants:write",
      ]);
      const otherUser = await createJwtClient("alice", ["me"]);
      ...
      const thread = await owner.threads.create();
      const input = {
        messages: [{ role: "human", content: "Checkpoint test" }],
      };
      ...
      const checkpointId =
        history[history.length - 1].checkpoint?.checkpoint_id;
      if (!checkpointId) {
  ...
  it.skipIf(process.version.startsWith("v18."))(
    ...
    async () => {
      ...
      const result = await owner.assistants.setLatest(
